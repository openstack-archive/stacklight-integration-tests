...
class OpenstackTelemeteryPluginApi(base_test.PluginApi):
    def __init__(self):
        ...
        self._heat_cli = None
        self.keystone_access = self.helpers.os_conn.keystone_access
    ...
    def _create_flavor(self, ram=256, vcpus=1, disk=2):
        """This method creates a flavor for Heat tests."""

        logger.info("Creating Heat test flavor...")
    ...
    def _create_keypair(self, name="ostf_test-keypair-autoscaling"):
        logger.info("Creating test keypair for Heat autoscaling...")
        keypair = None
    ...
    def check_ceilometer_autoscaling(self):
        ...
        self._check_required_resources(min_required_ram_mb=7000)
        ...
        keypair = self.helpers.verify(60, self._create_keypair, 2, fail_msg,
        ...
        instances = self.helpers.verify(60, self.get_instances_by_name_mask, 7,
                                        "Failed to get instances list",
                                        "getting instances list",
                                        mask_name=reduced_stack_name)

        # create floating ip
        floating_ip = self.helpers.verify(60, self._create_floating_ip, 8,
                                          "Failed to create floating ip",
                                          "creating floating ip")

        # assign floating ip to instance
        self.helpers.verify(60, self._assign_floating_ip_to_instance, 9,
                            "Failed to assign floating ip to instance",
                            "assigning floating ip to instance",
                            server=instances[0], floating_ip=floating_ip)
    ...
    def _create_internal_subnet(self, net, tenant_id):
        ...
        logger.info(
            'Creating subnet for net "{}"...'.format(net['name']))
        subnet_body = {
            'subnet': {
                'network_id': net['id'],
                'ip_version': 4,
        ...
        subnet = self.neutron_cli.create_subnet(subnet_body)[
            'subnet']

        return subnet

    def _create_router(self, name, ext_net, tenant_id):
        """This method creates a router.

        All resources created by this method will be automatically deleted.
        """
        logger.info('Creating router with name "{0}"...'.format(name))
        router_body = {
            'router': {
                'name': name,
        ...
        router = self.neutron_cli.create_router(router_body)[
            'router']
    ...
    def _create_network_resources(self, tenant_id):
        ...
        subnet = self._create_internal_subnet(net, tenant_id)
        router_name = 'ostf-autoscaling-test-service-router'
        router = self._create_router(router_name, ext_net, tenant_id)
