...
class TestToolchainLDAP(api.ToolchainApi):
    ...
    def deploy_toolchain_with_ldap_authz(self):
        ...
        Scenario:
            ...
            6. Upload install_slapd.sh script on controller node
            7. On controller node open the firewall for ports 389 and 636
            8. Install and configure the LDAP plugin
            9. Check that LMA Toolchain plugins are running
    ...
    def _create_ldap_toolchain(self, authz=False, protocol='ldap'):
        ...
        plugins_ldap = (self.ELASTICSEARCH_KIBANA, self.INFLUXDB_GRAFANA,
        ...
        self._activate_ldap_plugin(
            self.INFLUXDB_GRAFANA, "(uid=%s)", ldap_server, authz=authz,
            protocol=protocol)
        self._activate_ldap_plugin(
            self.ELASTICSEARCH_KIBANA, "(objectClass=*)", ldap_server,
            authz=authz, protocol=protocol)
        self._activate_ldap_plugin(
            self.LMA_INFRASTRUCTURE_ALERTING, "(objectClass=*)", ldap_server,
            authz=authz, protocol=protocol)
        ...
        for plugin in plugins_ldap:
            plugin.check_plugin_ldap(authz=authz)

    @staticmethod
    def _activate_ldap_plugin(plugin, ufilter, ldap_server, authz=False,
                              protocol="ldap"):
