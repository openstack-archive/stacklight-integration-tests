...
class TestToolchainLDAP(api.ToolchainApi):
    ...
    def deploy_toolchain_with_ldap(self):
        ...
        self._activate_ldap_plugin(self.INFLUXDB_GRAFANA, "(uid=%s)",
                                   ldap_server)
        self._activate_ldap_plugin(self.ELASTICSEARCH_KIBANA,
                                   "(objectClass=*)", ldap_server)
        self._activate_ldap_plugin(self.LMA_INFRASTRUCTURE_ALERTING,
                                   "(objectClass=*)", ldap_server)
    ...
    def _activate_ldap_plugin(plugin, ufilter, ldap_server, authz=False,
        ...
        logger.info(
            ...
            "user search filter: {3}, ldap server node: {4}".format(
                plugin.get_plugin_settings().name, authz, protocol, ufilter,
                ldap_server)
        )
