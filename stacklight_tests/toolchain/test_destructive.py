...
class TestLMAInfraAlertingPluginDestructive(api.ToolchainApi):
    ...
    @test(depends_on_groups=["prepare_slaves_9"],
          groups=["deploy_toolchain_load_ha", "deploy", "toolchain",
                  "destructive"])
    @log_snapshot_after_test
    def deploy_toolchain_load_ha(self):
        ...
        Duration 60m
        Snapshot deploy_toolchain_load_ha
        """

        self.check_run('deploy_toolchain_load_ha')

        self.env.revert_snapshot("ready_with_9_slaves")

        self.prepare_plugins()

        self.helpers.create_cluster(settings={'volumes_ceph': True,
                                              'images_ceph': True,
                                              'volumes_lvm': False,
                                              'osd_pool_size': "3"})
        ...
        logger.info('Making environment snapshot '
                    'deploy_toolchain_load_ha')
        self.env.make_snapshot("deploy_toolchain_load_ha", is_make=True)

    @test(depends_on_groups=["deploy_ha_toolchain"],
          groups=["simulate_network_failure", "network_failure", "toolchain",
                  "destructive"])
    @log_snapshot_after_test
    ...
    @test(depends_on_groups=["deploy_ha_toolchain"],
          groups=["simulate_network_interruption", "network_interruption",
    ...
    def restart_services_during_workload(self):
        ...
        self.env.revert_snapshot("deploy_toolchain_load_ha")
        ...
        process_list.append(multiprocessing.Process(
            target=self.load.create_load, args=(queue, cluster_id)))
        process_list.append(multiprocessing.Process(
