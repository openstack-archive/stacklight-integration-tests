...
class TestOpenstackTelemetry(api.ToolchainApi):
    ...
    @test(depends_on_groups=["prepare_slaves_5"],
          groups=["deploy_telemetry_ceilometer_without_LMA",
                  "deploy", "smoke"])
    @log_snapshot_after_test
    def deploy_telemetry_ceilometer_core_without_LMA(self):
        """Deploy an environment with Openstack-Telemetry plugin with enabled
        Resource API and Event API, Elasticsearch and InfluxDB backends,
        Kafka plugin and  Ceilometer Core, disabled LMA collector.
        ...
        self._deploy_telemetry_plugin(
            "deploy_telemetry_ceilometer_without_LMA",
            additional_plugins=self.KAFKA,
            disable_plugins=self.LMA_COLLECTOR,
            advanced_options=options,
            additional_tests=additional_tests,
            roles=self.ceilometer_core_kafka_roles, settings=settings)

    @test(depends_on_groups=["prepare_slaves_5"],
          groups=["deploy_telemetry_without_LMA",
                  "deploy", "smoke"])
    @log_snapshot_after_test
    def deploy_telemetry_without_LMA(self):
        """Deploy an environment with Openstack-Telemetry plugin with enabled
        Resource API and Event API, Elasticsearch and InfluxDB backends,
        Kafka plugin and Ceilometer Core, disabled LMA collector.
