...
class ToolchainApi(object):
    ...
    def check_alarms(self, alarm_type, source, value, time_point="now() - 5m"):
        measurement = "{}_status".format(alarm_type)
        query = (
            "select * from {} "
            "where time >= {} and source = '{}' and value = {}".format(
                measurement, time_point, source, value))

        def check_result():
            result = self.INFLUXDB_GRAFANA.do_influxdb_query(
                query=query).json()["results"][0]["series"]
            return len(result)

        msg = "Alarm with source {} and value {} was not triggered".format(
            source, value)
        devops_helpers.wait(check_result, timeout=60 * 3,
