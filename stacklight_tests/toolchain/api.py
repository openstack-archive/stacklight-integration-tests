...
class ToolchainApi(object):
    ...
    def restart_services_actions(self, queue, cluster_id):
        """Verify that all services respond correctly to management
         under workload. This method will attempt to move, shutdown and
         restart various services on cluster nodes under workload. Each
         action is followed by toolchain plugins state and data flow check.

        :param queue: multiprocessing queue to return exit state to the
         parent process.
        ...
        try:
            ...
            # Note (vushakov): temporary removing controller node with
            # vip__management service from the node list.
