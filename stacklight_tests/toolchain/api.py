    ...
    def restart_services_actions(self, queue, cluster_id):
        ...
        try:
            ...
            self.plugins_mapping[
                ...
                nailgun_compute, "restart")
            self.plugins_mapping[
                'lma_collector'].manage_lma_collector_service(
                nailgun_plugins, "restart")
            ...
            self.plugins_mapping[
                'lma_collector'].manage_lma_collector_service(
                nailgun_compute, "stop")
            self.plugins_mapping[
                'lma_collector'].manage_lma_collector_service(
                nailgun_plugins, "stop")
            logger.info("Checking PID of hekad on all nodes except"
                        " controllers")
            self.helpers.get_tasks_pids(['hekad'], nailgun_compute,
                                        1)
            self.helpers.get_tasks_pids(['hekad'], nailgun_plugins,
                                        1)
            logger.info("Starting lma_collector on all nodes except"
                        " controllers.")
            self.plugins_mapping[
                'lma_collector'].manage_lma_collector_service(
                nailgun_compute, "start")
            self.plugins_mapping[
                'lma_collector'].manage_lma_collector_service(
                nailgun_plugins, "start")
            self.monitoring_check()
