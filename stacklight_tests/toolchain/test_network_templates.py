...
class TestToolchainNetworkTemplates(api.ToolchainApi):
    ...
    def get_network_template(template_name):
        template = os.path.join(
            ...
            "stacklight-integration-tests/fixtures/network_templates/",
    ...
    def deploy_toolchain_with_network_template(self):
        ...
        Scenario:
            ...
            8. Configure the monitoring subnet.
        ...
        self.helpers.fuel_client.upload_network_template(
        ...
        # Hijack the management network's address space for monitoring
        networks = self.fuel_client.get_network_groups()
        logger.info("Networks definition before update: {0}".format(networks))
        networks["monitoring"] = copy.deepcopy(networks['management'])
        networks["management"]["ip_ranges"] = ["10.109.5.2", "10.109.5.254"]
        networks["management"]["cidr"] = "10.109.5.0/24"
        logger.info('Networks definition after update: {0}'.format(networks))

        self.helpers.verify_networks()
