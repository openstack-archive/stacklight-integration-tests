...
class TestToolchainNetworkTemplates(api.ToolchainApi):
    ...
    def deploy_toolchain_with_network_template(self):
        ...
        fuel_client = self.helpers.fuel_client
        network_template = self.get_network_template("monitoring")
        fuel_client.upload_network_template(
        ...
        networks = fuel_client.get_network_groups()
        ...
        asserts.assert_is_not_none(
            management_net, "Could find management network")
        monitoring_net = copy.deepcopy(management_net)
        monitoring_net["name"] = "monitoring"
        monitoring_net.pop("id")
        management_net["cidr"] = "10.109.5.0/24"
        management_net["meta"]["cidr"] = "10.109.5.0/24"
        management_net["meta"]["ip_range"] = ["10.109.5.2", "10.109.5.254"]
        fuel_client.update_network_group(management_net["id"], management_net)
        fuel_client.add_network_group(monitoring_net)
