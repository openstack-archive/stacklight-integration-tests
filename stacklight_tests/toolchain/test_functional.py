    ...
    def toolchain_warning_alert_service(self):
        ...
        Scenario:
            ...
            9. Repeat steps 2 to 8 for the following services:
                    ...
                    cinder-api services respectively).
            ...
            for key in services:
                for service in services[key]:
                    self.change_verify_service_state(
                        ...
                        new_state=['WARNING', 'WARN'],
                        down_nodes_in_influx=[1, 1],
                        toolchain_node=toolchain_node,
                        contoller_nodes=[controller_node],
                        nagios_driver=driver)
                    self.change_verify_service_state(
                        service_name=[service, key], action='start',
                        new_state=['OK', 'OKAY'], down_nodes_in_influx=[0, 0],
                        toolchain_node=toolchain_node,
                        contoller_nodes=[controller_node],
                        nagios_driver=driver)
        ...
        with self.ui_tester.ui_driver(url, "//frame[2]",
            ...
            for key in services:
                for service in services[key]:
                    ...
                    self.change_verify_service_state(
                        service_name=[service, key], action='stop',
                        new_state=['CRITICAL', 'CRIT'],
                        down_nodes_in_influx=[3, 2],
                        toolchain_node=toolchain_node,
                        contoller_nodes=[controller_nodes[0],
                                         controller_nodes[1]],
                        nagios_driver=driver)
                    self.change_verify_service_state(
                        service_name=[service, key], action='start',
                        new_state=['OK', 'OKAY'],
                        down_nodes_in_influx=[0, 0],
                        toolchain_node=toolchain_node,
                        contoller_nodes=[controller_nodes[0],
                                         controller_nodes[1]],
                        nagios_driver=driver)
    ...
    def toolchain_warning_alert_node(self):
        ...
        with self.ui_tester.ui_driver(url, "//frame[2]",
            ...
            self.change_verify_node_service_state(
                ...
                toolchain_node, [nailgun_nodes[0], nailgun_nodes[1]], driver)
