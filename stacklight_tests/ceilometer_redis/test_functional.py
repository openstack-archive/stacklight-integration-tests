...
class TestFunctionalCeilometerRedisPlugin(api.CeilometerRedisPluginApi):
    ...
    def central_agents_restart(self):
        ...
        with self.fuel_web.get_ssh_for_nailgun_node(controller) as remote:
            ...
            msg = "Error, agent central was't stopped"
    ...
    def check_samples(self):
        ...
        lis = self.ceilometer.statistics.list(q=query,
                                              meter_name='image')[0].count
        msg = "Error, Excpected 1 image sample, got : {0} .".format(lis)
        asserts.assert_true(lis == 1, msg)
