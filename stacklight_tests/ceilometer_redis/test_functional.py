...
class TestFunctionalCeilometerRedisPlugin(api.CeilometerRedisPluginApi):
    ...
    def central_agents_restart(self):
        ...
        msg = " Agent central was't stopped"
    ...
    def check_polling_logs(self):
        ...
        msg = "'Joined partitioning group agent-central' not found"
    ...
    def check_alarm_evaluator_logs(self):
        ...
        msg = "'Joined partitioning group alarm_evaluator' not found"
        asserts.assert_true(len(cmd) > 0, msg)

    @test(depends_on_groups=["deploy_ceilometer_redis"],
          groups=["check_samples", "ceilometer_redis",
                  "functional"])
    @log_snapshot_after_test
    def check_samples(self):
        ...
        f = datetime.now() - timedelta(seconds=600)
        ...
        msg = "We have more 1 image sample for one polling period" \
              ".Excpected 1 image sample, " \
              "got : {0} .".format(sample_count)
