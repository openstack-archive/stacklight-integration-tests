...
class PluginHelper(object):
            ...
            for node in self.fuel_web.client.list_cluster_nodes(self.cluster_id):
                for k in nodes_hostnames.keys():
                    if node['name'].startswith(k):
                        logger.info("Setting hostname for Node Id {0} Name {1} to {2}".format(
                            node['id'], k, nodes_hostnames[k]))
                        self.fuel_web.client.set_hostname(node['id'], nodes_hostnames[k])
        if verify_network:
    ...
    def verify_custom_hostnames(self, nodes_hostnames):
        ...
        for node in self.fuel_web.client.list_cluster_nodes(self.cluster_id):
            prev_node_name = filter(lambda(x): node['name'].startswith(x), nodes_hostnames.keys())
            asserts.assert_true(
                len(prev_node_name) > 0,
                "Can not retrieve any node for name {0}".format(node['name']))
            asserts.assert_true(
                len(prev_node_name) == 1,
                "Too many match for node name {0}: {1}".format(node['name'], prev_node_name))
            ...
            with self.env.d_env.get_admin_remote() as admin_remote:
                ...
                cmd = "ssh -o StrictHostKeyChecking=no -q {0} hostname -s".format(node['hostname'])
