...
from stacklight_tests.lma_infrastructure_alerting.test_functional \
    import TestLMAInfraAlertingPluginFunc as functional
...
class TestLMAInfraAlertingPlugin(api.InfraAlertingPluginApi):
    ...
    @test(depends_on=[functional.deploy_lma_infra_alerting_plugin_in_ha_mode],
    ...
    def plugin_core_repos_setup(self):
        ...
        Scenario:
            ...
            2.  Create an environment with enabled plugin in the
             Fuel Web UI and deploy it.
            3.  Run OSTF
            4.  Go in cli through controller / compute / storage
             /etc nodes and get pid of
            ...
            7.  Go to controller/plugin/storage node and check
            if plugin's services are
                alive and aren't changed their pid.
            8.  Check with fuel nodes command that all nodes are
...
# TODO: fuel-createmirror -M will fail during the execution. CHANGED!
            asserts.assert_equal(1, exec_res['exit_code'],
                                 'fuel-createmirror -M failed: {0}'
                                 .format(exec_res['stderr']))
            cmd = "fuel --env {0} node --node-id " \
                  "1 2 3 --tasks setup_repositories"\
                .format(self.helpers.cluster_id)
        ...
        for node in origina_pids:
            for process in origina_pids[node]:
                if origina_pids[node][process] != new_pids[node][process]:
                    logger.error("Process {0} on node {1} has changed "
                                 "its pid! Was: {2} Now: {3}"
                                 .format(process, node,
                                         origina_pids[node][process],
        ...
        with self.env.d_env.get_admin_remote() as remote:
            exec_res = remote.execute("fuel nodes | awk "
                                      "{'print $3'} | grep error")
# TODO: Nodes are in deploying state. CHANGED!
            asserts.assert_equal(1, exec_res['exit_code'],
                                 'Some nodes are in error state!')
