...
class TestLMAInfraAlertingPlugin(api.InfraAlertingPluginApi):
    ...
    def plugin_core_repos_setup(self):
        ...
        Scenario:
            ...
            if plugin's services are alive and aren't changed their pid.
        ...
        with self.env.d_env.get_admin_remote() as remote:
            exec_res = remote.execute("fuel nodes | awk {'print $3'}"
                                      " | grep 'error\|deploying'")
            asserts.assert_equal(1, exec_res['exit_code'],
                                 'Some nodes are in error\deploying state!')
        self.helpers.run_ostf()
