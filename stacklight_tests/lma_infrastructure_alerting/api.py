...
class InfraAlertingPluginApi(base_test.PluginApi):
    ...
    def lma_infrastructure_alerting_check(self, interval=600):
        """Check all active cluster nodes state. All nodes must be in
         UP state and the time of the last update must be within supplied
         time interval.

        :param interval: time interval for node state in nagios to be updated
         within. If the time of the last update is not within this interval,
         node state cannot be considered valid.
        :type interval: int.
        """
        ...
        with self.ui_tester.ui_driver(
            ...
            for node in nailgun_nodes:
                ...
                for index in range(
                        2, self.ui_tester.get_table_size(table) + 1):
                    node_name = self.ui_tester.get_table_cell(
