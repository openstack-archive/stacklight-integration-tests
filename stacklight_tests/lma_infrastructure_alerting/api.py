...
from stacklight_tests.influxdb_grafana import(
    plugin_settings as influxdb_plugin_settings)
from stacklight_tests.lma_collector import(
    plugin_settings as collector_plugin_settings)
...
class InfraAlertingPluginApi(base_test.PluginApi):
    ...
    def check_node_in_nagios(self, changed_node, state):
        ...
        try:
            ...
            asserts.assert_equal(state, self.ui_tester.node_is_present(
                driver, changed_node),
                "Failed to find node '{0}' on nagios!".format(changed_node))
        finally:
            driver.close()
            ...
            for key in node_services:
                if key == service:
                    asserts.assert_equal(
                        service_state[service], node_services[key],
                        "Wrong service state found on node {0}: expected"
                        " {1} but found {2}".format(node_name,
    ...
    def change_verify_service_state(self, service, action, state, lma_node,
        ...
        logger.info("Changing state of service {0}. "
                    "New state is {1}".format(service[0], state))
        ...
        self.helpers.check_local_mail(lma_node,
                                      "{0} is {1}".format(service[1], state))
