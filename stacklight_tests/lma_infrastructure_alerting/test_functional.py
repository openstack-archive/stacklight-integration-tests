...
class TestLMAInfraAlertingPluginFunc(api.InfraAlertingPluginApi):
    ...
    def add_remove_infrastructure_alerting(self):
        ...
        self.helpers.remove_node_from_cluster({'slave-05': [self.settings.role_name, 'influxdb_grafana']})
        self.run_ostf(should_fail=1)
        self.check_plugin_online()
        self.check_node_in_nagios(target_node, False)

        self.helpers.add_node_to_cluster({'slave-05': [self.settings.role_name, 'influxdb_grafana']})
        self.run_ostf(should_fail=1)
        self.check_plugin_online()
        target_node = self.helpers.get_fuel_node_name('slave-05')
...
#TODO: TEMPORARY! MUST BE TAKEN FROM INFLUXDB TEST SUITE
    ...
    def warning_alert_node_infra_alerting(self):
        ...
        service_nodes = self.fuel_web.get_devops_nodes_by_nailgun_nodes(nailgun_nodes)
        try:
            self.change_verify_node_service_state(['mysql', 'mysql-nodes.mysql-fs'], 'WARNING',
                                                  '96', lma_node, [service_nodes[0].name,
                                                                   service_nodes[1].name], driver)
        finally:
            driver.close()
    ...
    def critical_alert_node_infra_alerting(self):
        ...
        nailgun_nodes = self.fuel_web.get_nailgun_cluster_nodes_by_roles(
                        self.helpers.cluster_id, ['controller'])
        service_nodes = self.fuel_web.get_devops_nodes_by_nailgun_nodes(nailgun_nodes)
#TODO: All services except SSH management network will fall in UNKNOWN state. Test will fail on status check.
        try:
            self.change_verify_node_service_state(['mysql', 'mysql-nodes.mysql-fs'], 'CRITICAL',
                                                  '98', lma_node, [service_nodes[0].name,
                                                                   service_nodes[1].name], driver)
        finally:
            driver.close()
