...
class TestDestructiveZabbix(api.ZabbixApi):
    ...
    def test_host_failover(self):
        ...
        fqdn = self.get_node_with_zabbix_vip_fqdn()
        controller = self.fuel_web.get_devops_node_by_nailgun_fqdn(fqdn)

        self.helpers.power_off_node(controller)
        self.wait_zabbix_vip_migration(fqdn)
