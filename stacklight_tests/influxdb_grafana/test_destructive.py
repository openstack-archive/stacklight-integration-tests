...
class TestDestructiveInfluxdbPlugin(api.InfluxdbPluginApi):
    ...
    @test(depends_on_group=["deploy_ha_influxdb_grafana"],
          groups=["check_disaster_influxdb_grafana",
                  "influxdb_grafana", "destructive",
                  "emulate_network_disaster_whole_cluster_influxdb_grafana"])
    @log_snapshot_after_test
    def emulate_network_disaster_whole_cluster_influxdb_grafana(self):
    ...
    @test(depends_on_group=["deploy_ha_influxdb_grafana"],
          groups=["check_disaster_influxdb_grafana",
                  "influxdb_grafana", "destructive",
                  "emulate_network_disaster_on_influxdb_grafana_node"])
    @log_snapshot_after_test
    def emulate_network_disaster_on_influxdb_grafana_node(self):
