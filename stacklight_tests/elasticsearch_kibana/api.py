...
class ElasticsearchPluginApi(base_test.PluginApi):
    ...
    def make_instance_actions(self):
        ...
        flavors = self.helpers.os_conn.nova.flavors.list(sort_key="memory_mb")
        logger.info("Launch an instance")
        instance = self.helpers.os_conn.create_server_for_migration(
            label=net_name, flavor=flavors[0])
        logger.info("Update the instance")
        self.helpers.os_conn.nova.servers.update(instance, name="test-server")
        self.helpers.wait_for_resource_status(
            self.helpers.os_conn.nova.servers, instance, "ACTIVE")
        image = self.helpers.os_conn._get_cirros_image()
        logger.info("Rebuild the instance")
        self.helpers.os_conn.nova.servers.rebuild(
            instance, image, name="rebuilded_instance")
        self.helpers.wait_for_resource_status(
            self.helpers.os_conn.nova.servers, instance, "ACTIVE")
        logger.info("Resize the instance")
        self.helpers.os_conn.nova.servers.resize(instance, flavors[1])
        self.helpers.wait_for_resource_status(
            self.helpers.os_conn.nova.servers, instance, "VERIFY_RESIZE")
        logger.info("Confirm the resize")
        self.helpers.os_conn.nova.servers.confirm_resize(instance)
        self.helpers.wait_for_resource_status(
            self.helpers.os_conn.nova.servers, instance, "ACTIVE")
        logger.info("Resize the instance")
        self.helpers.os_conn.nova.servers.resize(instance, flavors[2])
        self.helpers.wait_for_resource_status(
            self.helpers.os_conn.nova.servers, instance, "VERIFY_RESIZE")
        logger.info("Revert the resize")
        self.helpers.os_conn.nova.servers.revert_resize(instance)
        self.helpers.wait_for_resource_status(
            self.helpers.os_conn.nova.servers, instance, "ACTIVE")
        logger.info("Stop the instance")
        self.helpers.os_conn.nova.servers.stop(instance)
        self.helpers.wait_for_resource_status(
            self.helpers.os_conn.nova.servers, instance, "SHUTOFF")
        logger.info("Start the instance")
        self.helpers.os_conn.nova.servers.start(instance)
        self.helpers.wait_for_resource_status(
            self.helpers.os_conn.nova.servers, instance, "ACTIVE")
        logger.info("Suspend the instance")
        self.helpers.os_conn.nova.servers.suspend(instance)
        self.helpers.wait_for_resource_status(
            self.helpers.os_conn.nova.servers, instance, "SUSPENDED")
        logger.info("Resume the instance")
        self.helpers.os_conn.nova.servers.resume(instance)
        self.helpers.wait_for_resource_status(
            self.helpers.os_conn.nova.servers, instance, "ACTIVE")
        logger.info("Create an instance snapshot")
        snapshot = self.helpers.os_conn.nova.servers.create_image(
            instance, "test-image")
        self.helpers.wait_for_resource_status(
            self.helpers.os_conn.nova.images, snapshot, "ACTIVE")
        logger.info("Delete the instance")
        self.helpers.os_conn.nova.servers.delete(instance)
        logger.info("Check that the instance was deleted")
        self.helpers.os_conn.verify_srv_deleted(instance)
        return instance.id
