...
from stacklight_tests.elasticsearch_kibana import test_smoke_bvt


@test(groups=["plugins"])
class TestNodesInfluxdbPlugin(api.ElasticsearchPluginApi):
    """Class for system tests for Elasticsearch-Kibana plugin."""

    @test(depends_on=[test_smoke_bvt.
          TestElasticsearchPlugin.deploy_ha_elasticsearch_kibana_plugin],
          groups=["check_scaling_elasticsearch_kibana", "scaling",
                  "elasticsearch_kibana", "system",
                  "check_add_delete_controller_elasticsearch_kibana"])
    ...
    @test(depends_on=[test_smoke_bvt.
          ...
          groups=["check_scaling_elasticsearch_kibana", "scaling",
                  "elasticsearch_kibana", "system",
                  "check_add_delete_compute_elasticsearch_kibanaa"])
    @log_snapshot_after_test
    ...
    @test(depends_on=[test_smoke_bvt.
          ...
          groups=["check_scaling_elasticsearch_kibana", "scaling",
                  "elasticsearch_kibana", "system",
                  "check_add_delete_elasticsearch_kibana_node"])
    @log_snapshot_after_test
